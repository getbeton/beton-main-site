...
integrations:
  ...
  - "pipedrive"
faq:
  - question: "What signals does Beton fire for usage-based upsell opportunities?"
    answer: "Five categories: approaching plan limits (70/80/90% consumed), overage events (hitting rate limits or paying overage fees), sustained growth trajectory (steady consumption growth that suggests a higher tier is coming), feature-tier lockouts (user attempted a feature gated to a higher plan), and pricing-page visits (often a procurement signal before renewal)."
  - question: "Why not just run a monthly 'accounts approaching plan limit' report?"
    answer: "Three problems with the monthly cadence: (1) the customer has usually already decided by the time the report goes out, (2) a Slack list pulls reps out of their CRM and most accounts on it never get touched, (3) a single 'limit approaching' signal flattens what should be five distinct outreach scenarios. Beton routes the signal in real-time, with metadata, into the CRM."
  - question: "Does Beton tell us when customers are downgrading or working around our limits?"
    answer: "Yes — the 'self-serve workaround' pattern (creating a second account, batching API calls, manual capping) shows up as a behavior change in PostHog. Beton can flag it. This is one of the most underused expansion signals because most teams never instrument workaround detection explicitly."
  - question: "What CRMs work for usage-based upsell signals?"
    answer: "Attio, HubSpot, Zoho, Pipedrive natively. Webhook for everything else. The signal goes onto the existing account record, not into a separate dashboard, so the AE or CSM sees it where they already work."
  - question: "How is this different from Pocus or MadKudu?"
    answer: "Pocus and MadKudu require sending data to their cloud and operate on rules or models you define. Beton sits on top of your existing PostHog (no data migration) and discovers the patterns automatically. It's open source, so the heuristic is auditable. And the price model is per-tracked-user, not per-seat."
